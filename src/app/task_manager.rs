...
impl TaskManager {
    ...
    /// Get a formatted status line showing task progress.
    /// When multiple tasks are running, shows a summary like "[1/3] Copying 3/15 (125M/298M)"
    pub fn status_line(&self) -> Option<String> {
        let running_count = self.active_count();
        if running_count == 0 {
            return None;
        }

        // Show the latest running task's progress with a task counter prefix
        let task = self
            .tasks
            .iter()
            .rev()
        ...
        {
            let bar = crate::util::progress_bar(*progress_pct, 15);
            let elapsed = task.started_at.elapsed();
            let eta = self.estimate_eta(task, elapsed);
            let eta_str = eta
                .map(|d| format!(" ETA {}", crate::util::format_duration(d)))
                .unwrap_or_default();

            if running_count > 1 {
                return Some(format!(
                    "[1/{running_count}] {status_text} {bar} {progress_pct}%{eta_str}"
                ));
            } else {
                return Some(format!(
                    "{status_text} {bar} {progress_pct}%{eta_str}"
                ));
            }
        }
    ...
    fn estimate_eta(&self, task: &Task, elapsed: std::time::Duration) -> Option<std::time::Duration> {
        if let TaskState::Running { progress_pct, .. } = &task.state {
            let pct = *progress_pct;
            if pct > 0 && pct < 100 && elapsed.as_secs_f64() > 0.5 {
                let remaining_pct = 100.0 - pct as f64;
                let rate = pct as f64 / elapsed.as_secs_f64();
                let eta_secs = remaining_pct / rate;
                return Some(std::time::Duration::from_secs_f64(eta_secs));
            }
        }
        None
    }
