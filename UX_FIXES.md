⏺ Сделал свежий проход по коду. TODO/FIXME нет (проект чистый), но видны конкретные пробелы. Вот что осталось, по
  ...
  удаление, trash-vs-permanent, ширина дерева/панелей, дефолтный редактор. Один раз сделать каркас чтения — и снять кучу
  «магических» дефолтов.
  ...
  7. [DONE] Восстановление из корзины — добавлены: трекинг удалённого в trash за сессию, оверлей
  `:trash`/`:restore` (Sp+r) со списком и восстановлением (Enter/r — один, R — все), и работающий
  откат через `u`. Кроссплатформенно: на Linux/Windows через `trash::os_limited`, на macOS — через
  snapshot-diff `~/.Trash` с обратным перемещением файла.
